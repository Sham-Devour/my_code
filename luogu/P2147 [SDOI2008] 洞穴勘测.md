...
int main() {
	...
	for (int i = 1; i <= m; i++) {
		...
		if (!vis[mp(u, v)]) vis[mp(u, v)] = i;
		else e[++cnt_edge] = edge(vis[mp(u, v)], i, u, v), vis[mp(u, v)] = 0;
	}
	for (int i = 1; i <= m; i++) {
		if (q[i].op == 2) continue;
		int u = Min(q[i].u, q[i].v), v = Max(q[i].u, q[i].v);
		if (vis[mp(u, v)]) e[++cnt_edge] = edge(vis[mp(u, v)], m, u, v), vis[mp(u, v)] = 0;
	}
	seg.build(1, 1, m);
